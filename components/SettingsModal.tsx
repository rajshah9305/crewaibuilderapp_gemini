...
export const SettingsModal: React.FC<SettingsModalProps> = ({ settings, setSettings, onClose }) => {
  ...
  return (
    <div className="fixed inset-0 bg-black bg-opacity-70 flex items-center justify-center z-50 animate-fade-in" onClick={onClose}>
      <div className="bg-light-panel dark:bg-dark-panel/80 backdrop-blur-md rounded-xl shadow-2xl p-8 w-full max-w-md border border-light-border dark:border-dark-border text-light-text-primary dark:text-dark-text-primary" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="space-y-6">
          <div>
            ...
            <div className="flex items-center space-x-2 bg-light-bg dark:bg-dark-bg/50 p-3 rounded-md border border-light-border dark:border-dark-border">
              <span className="text-green-400">●</span>
              <span className="font-mono text-sm">Securely Configured</span>
            </div>
            <p className="text-xs text-light-text-secondary dark:text-dark-text-secondary mt-2 p-2 bg-slate-100 dark:bg-dark-panel/50 rounded-md">
          ...
          <div>
            ...
            <select
                ...
                className="w-full bg-light-bg dark:bg-dark-bg/50 p-3 rounded-md border border-light-border dark:border-dark-border focus:ring-2 focus:ring-dark-accent-primary focus:outline-none"
            >
          ...
          <div>
            <label htmlFor="temperature" className="block text-sm font-medium text-light-text-secondary dark:text-dark-text-secondary mb-2">
              Temperature: <span className="font-bold text-dark-accent-primary">{settings.temperature.toFixed(2)}</span>
            </label>
            <input
              id="temperature"
              type="range"
              ...
              className="w-full h-2 bg-slate-300 dark:bg-slate-700 rounded-lg appearance-none cursor-pointer accent-dark-accent-primary"
            />
        ...
        <div className="mt-8 text-right">
          <button
            onClick={onClose}
            className="bg-dark-accent-primary hover:bg-dark-accent-primary-hover text-white font-semibold py-2 px-6 rounded-lg transition-colors"
          >
            Done
          </button>
        </div>
